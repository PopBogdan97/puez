...
class BLEThing:
    ...
    time_date = service1.characteristic(uuid = 7, value = 'TIME')
    ...
    def getTime(self):
        return self.time_date.value()

    def blaster_cb(self,chr):
        print("Read request, returning value = {}".format(chr.value()))
        if self.cb:
            self.cb(chr.value())

    def setCallBack(self, cb):
        self.cb = cb
        self.time_date.callback(trigger = Bluetooth.CHAR_WRITE_EVENT, handler = self.blaster_cb)
