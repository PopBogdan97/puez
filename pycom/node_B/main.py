...
while(True):
    ...
    if(rx):
        ...
        try:
            rxPkt = ujson.loads(rx)
            accepted = ['A'] # List of accepted ids
            if(rxPkt['id'] in accepted): # This data is yours, hold it and send it
                print("A package has arrived!")
                # idx = ['id', 'F', 'L', 'T', 'H', 'W', 'D']
                # for i in idx:
                #     i = rxPkt[i]
                #     print(i)
                ...
                globalPkt = ujson.dumps(globalDict)
                # Uplink sending
                s.send(globalPkt)

                print(globalPkt + " sent")
                time.sleep(6) # this dead-time is needed by the network to gather to your LoRa packets
        except ValueError:
            pass
